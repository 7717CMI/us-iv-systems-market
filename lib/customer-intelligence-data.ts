...
// Realistic customer name generators by end user type (U.S. IV Systems Market)
const hospitalNames = [
  'Medical Center', 'General Hospital', 'University Hospital', 'Regional Medical Center',
  'Memorial Hospital', 'Community Hospital', 'Health System', 'Medical Center & Hospital'
]

const clinicNames = [
  'Outpatient Clinic', 'Specialty Clinic', 'Infusion Clinic', 'Medical Clinic',
  'Primary Care Center', 'Urgent Care Center', 'Oncology Clinic', 'Vascular Clinic'
]

const ascNames = [
  'Surgery Center', 'Ambulatory Surgery Center', 'Surgical Center', 'Day Surgery Center',
  'Outpatient Surgery', 'Surgical Associates', 'Same-Day Surgery Center', 'Surgical Suites'
]

const homeHealthNames = [
  'Home Health Services', 'Home Infusion Pharmacy', 'Home Care Services', 'Infusion Services',
  'Home Healthcare Agency', 'IV Therapy at Home', 'Home Nursing Services', 'Homecare Solutions'
]

const ltcNames = [
  'Skilled Nursing Facility', 'Long-Term Care Center', 'Nursing Home', 'Rehabilitation Center',
  'Extended Care Facility', 'Senior Care Center', 'Post-Acute Care', 'Continuing Care Center'
]

const emergencyNames = [
  'Trauma Center', 'Emergency Department', 'Emergency Care Center', 'Level I Trauma Center',
  'Emergency Services', 'Critical Care Center', 'Urgent & Emergency Care', 'Emergency Medical Center'
]

// Region-specific city/location prefixes for U.S. regions
const regionPrefixes: Record<string, string[]> = {
  'Northeast': ['Boston', 'New York', 'Philadelphia', 'Hartford', 'Providence', 'Albany', 'Newark'],
  'Southwest': ['Phoenix', 'Tucson', 'Albuquerque', 'Las Vegas', 'El Paso', 'Mesa', 'Scottsdale'],
  'West': ['Los Angeles', 'San Francisco', 'Seattle', 'Portland', 'Sacramento', 'San Diego', 'Denver'],
  'Southeast': ['Atlanta', 'Miami', 'Charlotte', 'Nashville', 'Orlando', 'Tampa', 'Raleigh'],
  'Midwest': ['Chicago', 'Detroit', 'Cleveland', 'Indianapolis', 'Minneapolis', 'Columbus', 'St. Louis']
}

function generateCustomerName(region: string, endUserSegment: string, index: number): string {
  const cities = regionPrefixes[region] || ['Regional', 'Metro', 'City']
  const city = cities[index % cities.length]

  let baseName = ''
  if (endUserSegment === 'Hospitals') {
    baseName = hospitalNames[index % hospitalNames.length]
  } else if (endUserSegment === 'Clinics') {
    baseName = clinicNames[index % clinicNames.length]
  } else if (endUserSegment === 'Ambulatory Surgical Centers') {
    baseName = ascNames[index % ascNames.length]
  } else if (endUserSegment === 'Home Healthcare Settings') {
    baseName = homeHealthNames[index % homeHealthNames.length]
  } else if (endUserSegment === 'Long-Term Care Facilities') {
    baseName = ltcNames[index % ltcNames.length]
  } else if (endUserSegment === 'Emergency & Trauma Care Settings') {
    baseName = emergencyNames[index % emergencyNames.length]
  } else {
    baseName = 'Research & Academic Institute'
  }
...
function generateCustomerCount(region: string, endUserSegment: string): number {
  // Base multipliers by U.S. region (reflecting healthcare facility density)
  ...
  // Base multipliers by end user type (Hospitals most common)
  const segmentMultipliers: Record<string, number> = {
    'Hospitals': 1.5,
    'Clinics': 2.0,
    'Ambulatory Surgical Centers': 1.8,
    'Home Healthcare Settings': 1.2,
    'Long-Term Care Facilities': 1.6,
    'Emergency & Trauma Care Settings': 0.6,
    'Others (Academics & Research Institutes & Others)': 0.5
  }
...
export function generateCustomerIntelligenceData(): CustomerIntelligenceData[] {
  ...
  const endUserSegments = [
    ...
    'Emergency & Trauma Care Settings',
    'Others (Academics & Research Institutes & Others)'
  ]
    ...
    endUserSegments.forEach(endUserSegment => {
      ...
      for (let i = 0; i < customerCount; i++) {
        customers.push({
          ...
          region,
          endUserSegment,
          type: endUserSegment === 'Hospitals' ? 'commercial'
                : endUserSegment === 'Home Healthcare Settings' ? 'residential'
                : 'utility'
        })
      }
